...
def test_create_curriculum(mock_db_session):
    """
    ...
    created_curriculum = curriculum_service.create_curriculum(mock_db_session, curriculum_data)
...
def test_get_curriculum(mock_db_session):
    """
    ...
    retrieved_curriculum = curriculum_service.get_curriculum(mock_db_session, test_uuid)

    mock_db_session.query.assert_called_once_with(Curriculum)
    mock_db_session.query.return_value.filter.assert_called_once()
    assert retrieved_curriculum == mock_curriculum

def test_get_curriculum_not_found(mock_db_session):
    """
...
def test_update_curriculum(mock_db_session):
    """
    ...
    updated_curriculum = curriculum_service.update_curriculum(mock_db_session, test_uuid, update_data)
...
def test_update_curriculum_not_found(mock_db_session):
    """
...
def test_delete_curriculum(mock_db_session):
    """
    ...
    deleted_curriculum = curriculum_service.delete_curriculum(mock_db_session, test_uuid)

    mock_db_session.delete.assert_called_once_with(existing_curriculum)
    mock_db_session.commit.assert_called_once()
    assert deleted_curriculum == existing_curriculum

def test_delete_curriculum_not_found(mock_db_session):
    """
