...
def create_test_curriculum(db_session: Session) -> str:
    ...
    curriculum_id = test_curriculum.curriculum_id  # Save before detaching
    return curriculum_id

def create_test_node(client: TestClient, curriculum_id: str, title: str, parent_node_id: str = None) -> dict:
    node_data = {"title": title, "parent_node_id": parent_node_id}
    response = client.post(f"/api/v1/curriculums/{curriculum_id}/nodes", json=node_data)
    assert response.status_code == 201, response.json()
    return response.json()

def get_node_map(client: TestClient, curriculum_id: str) -> dict:
    response = client.get(f"/api/v1/curriculums/{curriculum_id}")
    assert response.status_code == 200, f"Failed to get curriculum: {response.json()}"
...
def test_reorder_nodes_move_forward_same_parent(client: TestClient, db_session: Session):
    ...
    node1 = create_test_node(client, curriculum_id, "Node 1")
    node2 = create_test_node(client, curriculum_id, "Node 2")
    node3 = create_test_node(client, curriculum_id, "Node 3")
    ...
    node_map = get_node_map(client, curriculum_id)
...
def test_reorder_nodes_change_parent(client: TestClient, db_session: Session):
    curriculum_id = create_test_curriculum(db_session)
    parent_node = create_test_node(client, curriculum_id, "Parent")
    child_node = create_test_node(client, curriculum_id, "Child", parent_node_id=parent_node["node_id"])
    node_to_move = create_test_node(client, curriculum_id, "Node to Move")
...
def test_reorder_nodes_circular_dependency(client: TestClient, db_session: Session):
    curriculum_id = create_test_curriculum(db_session)
    parent = create_test_node(client, curriculum_id, "Parent")
    child = create_test_node(client, curriculum_id, "Child", parent_node_id=parent["node_id"])
