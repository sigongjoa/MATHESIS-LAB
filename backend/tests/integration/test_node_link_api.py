...
def create_test_curriculum(db_session: Session) -> str:
    ...
    return test_curriculum.curriculum_id  # Return string, not object

def create_test_node(client: TestClient, curriculum_id: str) -> dict:
...
def test_create_youtube_link(client: TestClient, db_session: Session):
    ...
    node = create_test_node(client, curriculum_id)
    ...
    video = db_session.query(YouTubeVideo).filter(YouTubeVideo.youtube_video_id == link["youtube_video_id"]).first()
...
def test_create_zotero_link(client: TestClient, db_session: Session):
    ...
    zotero_item_id = zotero_item.zotero_item_id  # Save before detaching
