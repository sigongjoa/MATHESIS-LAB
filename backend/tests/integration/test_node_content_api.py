...
# Helper function to create a node for testing node content
...
def test_read_node_content_not_found(client: TestClient):
    non_existent_node_id = uuid4()
    response = client.get(f"/api/v1/nodes/{non_existent_node_id}/content")
    assert response.status_code == 404
